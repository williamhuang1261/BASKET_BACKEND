...
import isSupplier from "../middleware/isSupplier.js";
import geocoder from "../utils/geocoder.js";
import User from "../models/users.js";
import  { valUserLocCreate } from "../models/locations.js";
import Supplier from "../models/suppliers.js";
import mongoose from "mongoose";
import express from "express";
import { getAuth } from "firebase-admin/auth";
import checkAuthPassword from "../utils/modifAuth.js";
...
router.post("/oauth", isLoggedIn, async (req: UserRequest, res: Response) => {
  ...
  if (!idToken) return res.status(400).send("No token provided");

  // Decode token to later access user data
  let decodedToken;
  try {
    // VerifyIdToken returns a DecodedIdToken object, or throws and error
...
router.put("/me", isLoggedIn, async (req: UserRequest, res: Response) => {
  const user = req.user!;
